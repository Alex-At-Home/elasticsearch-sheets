...
/** Add a new bucket form to an empty list */
QUnit.test( "Add new forms of each type", function(assert) {
  ...
  Fixtures.withParentDivAndGlobalEditor(testRoot, extraDivs, {}, function(globalEditor) {
    ...
    assert.equal(formElements.length, 4, "Added aggregation form")
...
QUnit.test( "Add existing metric form", function(assert) {
  var done = assert.async();
  ...
  Fixtures.withParentDivAndGlobalEditor(testRoot, "", {}, function(globalEditor) {
    ...
    assert.equal($(`#${nameId}`).val(), jsonForm.name, "Name correctly populated")
    assert.equal($(`#${nameId}`).attr('readonly'), "readonly", "Name is readonly")
    var aggTypeId = formId.replace("form_", "agg_type_")
    assert.equal($(`#${aggTypeId}`).val(), jsonForm.agg_type, "Agg Type correctly populated")
    var aggTypeId = formId.replace("form_", "help_agg_type_")
    expectedHelpUrl = AggregationInfo.metric.avg.url__
    assert.equal($(`#${aggTypeId} a`).attr('href'), expectedHelpUrl, "Agg Type has help link")
    var fieldFilterId = formId.replace("form_", "field_filter_")
    assert.equal($(`#${fieldFilterId}`).val(), jsonForm.field_filter, "Filter Field correctly populated")
    var locationId = formId.replace("form_", "location_")
    assert.equal($(`#${locationId}`).val(), jsonForm.location, "Filter Field correctly populated")
