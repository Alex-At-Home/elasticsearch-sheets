...
function TESTbuildSpecialRowInfo_(testSheet, testResults) {
   ...
   var buildInput = function(code, merge) {
      var tmp = deepCopyJson_(baseUiInput)
      if (code.indexOf("Q") >= 0) {
   ...
   performTest_(testResults, "check_defaults", function() {
      assertEquals_(inputVsOutputNoMerge[""], buildSpecialRowInfo_({}), "{}")
      var pqNoSource = deepCopyJson_(buildInput("PQ", /*merge*/false))
      ...
      assertEquals_(inputVsOutputNoMerge[""], buildSpecialRowInfo_(pqNoSource), "{}")
   })
   // Now check all combos:
   performTest_(testResults, "combo_test_nomerge", function() {
      for (var subTest in inputVsOutputNoMerge) {
         var input = buildInput(subTest, /*merge*/false)
         assertEquals_(inputVsOutputNoMerge[subTest], buildSpecialRowInfo_(input), subTest + "[" + JSON.stringify(input) + "]")
      }
   })
   performTest_(testResults, "combo_test_merge", function() {
      ...
      var inputVsOutputMerge = deepCopyJson_(inputVsOutputNoMerge)
      ...
      for (var subTest in inputVsOutputMerge) {
         ...
         assertEquals_(inputVsOutputMerge[subTest], buildSpecialRowInfo_(input), subTest + "[" + JSON.stringify(input) + "]")
      }
   })
}
